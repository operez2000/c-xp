  ...
  try {
    qry = `
      SELECT
        ...
        CASE WHEN clasificacion='Q' THEN
          CASE WHEN txtEmail IS NULL AND cierre IS NULL THEN 'Pend'
              WHEN txtEmail IS NOT NULL AND cierre IS NULL THEN 'Seg'
            WHEN txtEmail IS NOT NULL AND cierre IS NOT NULL THEN 'Cierre'
            ELSE 'Pend'
          END
          ELSE ''
        END estatus,
...
app.get("/api/responsables", mdi, (req, res) => {
  ...
  let qry = `
    ...
    FROM responsables
    WHERE estatus IS NULL OR estatus = 'A'
    ORDER BY nombre
  `
  sql.query(qry).then(([results]) => {
    json.result = 200
